...
async def formatter_node(state: ResearchState) -> Dict[str, Any]:
    ...
    synthesis_raw = state.get("raw_synthesis")
    if isinstance(synthesis_raw, SynthesisOutput):
        synthesis = synthesis_raw.model_dump()
    elif hasattr(synthesis_raw, "model_dump"):
        synthesis = synthesis_raw.model_dump()
    ...
    validation_audit = state.get("validation_result") or ValidationResult(
        is_valid=is_valid,
    ...
    research_report = ResearchReport.model_validate(
        {
            ...
            "executive_conclusion": synthesis.get("executive_conclusion")
            or raw.get("summary")
            ...
            "bull_case": synthesis.get("bull_case") or [],
            "bear_case": synthesis.get("bear_case") or [],
            "risk_factors": synthesis.get("risk_factors") or [],
            "financial_metrics": financial_metrics,
            "derived_metrics": calculated_metrics,
            "evidence": evidence,
            "evidence_chain": [],
            "validation_audit": validation_audit,
            "aggregate_confidence_score": float(synthesis.get("confidence_score") or 0.85),
